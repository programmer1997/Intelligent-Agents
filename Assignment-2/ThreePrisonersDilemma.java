public class ThreePrisonersDilemma {
	...
	class Marathe_Ajinkya_Player extends Player {
		// counters for total payoff of each player
		double myPayoff=0;
		double opp1Payoff=0;
		double opp2Payoff=0;
		...
		int selectAction(int n, int[] myHistory, int[] oppHistory1, int[] oppHistory2){
			...
			if (n==0) return 0;
			// Add the payoff from the last round to compute the total payoff till now for every player
			// *** This methods want to access the payoff 3D  matrix in the main class
			myPayoff+=payoff[myHistory[n-1]][oppHistory1[n-1]][oppHistory2[n-1]];
			opp1Payoff+=payoff[oppHistory1[n-1]][myHistory[n-1]][oppHistory2[n-1]];
			opp2Payoff+=payoff[oppHistory2[n-1]][myHistory[n-1]][oppHistory1[n-1]];

			// If my player is winning against both the opponent, i.e. payoff is largest then cooperate
			if((myPayoff>=opp1Payoff)&&(myPayoff>=opp2Payoff)){
				return 0;
			}
			// If winning against only opponent 1, cooperate if the last move of opponent 2 was cooperate, otherwise defect
			else if(myPayoff>opp1Payoff){

				if(oppHistory2[n-1]==0) return 0;
				else return  1;
			}
			// If winning against only opponent 2, cooperate if the last move of opponent 1 was cooperate, otherwise defect
			else if(myPayoff>opp2Payoff){

				if(oppHistory1[n-1]==0) return 0;
				else return  1;
			}

			// If losing against both, i.e. payoff is the lowest, then defect
			else return 1;
...
//	This is a helper function needed by scoresOfMatch.
	...
	Player makePlayer(int which) {
		...
		case 6: return new Marathe_Ajinkya_Player();
		}
